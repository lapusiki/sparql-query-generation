...
public class MapPredicateService implements PredicateService {
    ...
    private Map<String, List<String>> predicates = new HashMap<String, List<String>>() {{
        ...
        this.put(PredicateType.INTEREST.getValue(), Lists.asList("умеет писать", "смотрел", "знать, любить, читать, слушать, уметь, посещать, смотреть, интересоваться, изучать, покупать, брать, писать, сочинять, снимать, увлекаться, помнить, выбирать, обсуждать, вдохновлять, интриговать, заинтриговать, иметь".split(", ")));
